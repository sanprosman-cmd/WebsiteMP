...
1. **The MP logo asset** — `assets/img/logo-transparent.png` (rendered mark), `assets/img/MP Logo.svg`, `assets/img/New logo.png`, `assets/img/favicon.svg`. It is a transparent, padding-cropped wordmark sized *from* `--brand-h`; its color comes from the header's `currentColor` inversion, not from an edited file. The favicon is the logo's slash mark — the red tick over the green stroke that divides `maharani` from `prima` — reduced to a vector icon on the original cream plate `#F8F5EF` (the old-favicon ground — a dark tile disappears against dark tab strips): `favicon.svg` (64 viewBox, tile `rx 13`, strokes at 21° tilt, brand colors `#F02028` / `#089048`), with `favicon.png` (64px) and `apple-touch-icon.png` (180px) drawn from the same geometry as the raster delivery; every page references the PNG pair behind the shared `?v=` cache-bust and the web manifest lists the same two icons — `favicon.svg` stays on disk as the unreferenced vector master. The full wordmark never works as a tab icon — at 16px it renders as mush — so reduce to the slash; never redraw the wordmark by hand or swap in a placeholder mark.
   *Verify:* scroll `index.html` slowly past 32px — the mark must be white over the hero and unfiltered ink on the cream bar, never a boxed stamp, never missing on either ground; the tab icon must show the red/green slash on the cream plate over both light and dark tab strips.
...
5. **The footprints globe** — the canvas sphere in *Our Footprints* on `index.html`. Hand-rolled 2D orthographic projection, **no globe library and no WebGL**: land, borders, place names and markers all pass through the same `project()`, which is the only reason they cannot drift apart. Geometry is `assets/geo/earth-50m.js` — Natural Earth 1:50m land plus admin-0 boundaries (public domain), simplified offline against a vertex budget, shipped as a classic script that assigns `window.EARTH_50M` and lazy-injected on first intersection, so the sphere draws identically when served and when `index.html` is opened straight from disk, where a `fetch()` of the payload is blocked by the browser (the old empty-disc failure mode); if the script fails the graticule, island names and markers still draw. Each polygon carries a bounding cap so the far side is culled with one cosine — `tools/_geo/build-earth.js` audits that cull at 60 view centres across the reachable globe and refuses to write if any of them would visibly drop a polygon — and the archipelago gets three times the fidelity of the rest of the world. Three layers: land, the interior country-border mesh, and Indonesia lifted brighter than its neighbours — the sites are Indonesian, and a coastline alone does not say so; it is the border mesh, not text, that says *which* country. Names on the sphere are **islands only** (`SUMATRA`, `JAVA`, `SULAWESI`, `PAPUA`) at `.30` alpha with a `minV` horizon guard, under the shading and under the markers — country and continent text came off when the borders took over their job. It is achromatic — land `#3b3b43`, Indonesia `#5d5d67`, coasts `.30` white at 0.8px, borders `.40` white at 0.85px, Indonesia's outline `.55` at 1px, against a `#17171b → #08080a` body, white markers dominant. The drag gain is `1/R` radians per pointer pixel and rotation is **free**: longitude is unbounded (whole turns are shed from the state together, so the view never jumps), latitude is clamped to ±85°, and a fast release carries momentum — capped ±240°/s longitude, ±120°/s latitude, decaying like `e^-2.2t`, suppressed entirely under reduced motion and killed by a press-and-hold — so the visitor can spin to any country and back. A hemisphere-clipped land fill closes around the limb along its hidden vertices' azimuths, never as a straight chord across the disc, and that closure is construction: filled, never stroked as coast. Loop freezes off-screen; with reduced motion it parks at its pose and repaints only on pointer, resize, settled fonts or late geometry.
   *Verify:* on `index.html`, scroll to the plate — Sumatra through Halmahera must be recognisable, and every white site dot must read as sitting on land or on its coast. Judged in rendered pixels, not decimal degrees: the plate draws ~28 km per pixel, so Surabaya at 1.3 km off Java's north coast in the source is a thirtieth of a pixel from the drawn coastline and *looks* on it, while Karimun Jawa (88 km into the Java Sea, 3.1 px) and Matindok (29 km, 1.0 px) are offshore work and sit visibly clear of the coast — that is correct, not a defect. `tools/_geo/build-earth.js` measures all 25 markers against the raw Natural Earth source and the shipped payload and refuses to write if any coast has moved under one by more than a quarter pixel, so this cannot regress silently; do not replace that check with an exact point-in-polygon test, which reports five failures that cannot be seen. The Indonesia/Malaysia border on Borneo and the Indonesia/PNG border on New Guinea must read as hairlines *inside* those islands, not only around them; island names must sit on the sphere and turn away with the surface when dragged. Spin the globe hard in any direction — it must travel past Indonesia to the Americas and back without springing home, coast after a fast release and stop without a snap; park it on a mid-ocean view (the North Atlantic is the sharpest test) and land must end at the disc edge, never paint a straight-edged wedge of land across the water; the palette must stay black/white/grey with no blue anywhere; open `index.html` via `file://` and the land, borders and markers must still draw.
