...
4. **The site record deck** — the slide-deck portfolio on `projects.html`: ten plates `assets/img/photos/1…10.webp` (jpg masters alongside), stacked right-to-left on a desk. Position is a `data-pos` slot from `-1` to `5` that JS recomputes as the shortest wrapped path, and every state is a CSS transition on **transform and opacity only** — no keyframes, no layout property. Travel is `--deck-move: 460ms` on `--ease-in-out`, the leave mirrors the arrival path, and the caption cross-fades in the same grid cell so the block never reflows. Controls are 48px pill arrows plus a numbered rail with `aria-current`, and the whole thing is `[data-deck]`-guarded so it is inert without the markup.
   *Verify:* on `projects.html`, hold the right arrow down to hammer the deck — plates must retarget mid-flight rather than queue or jump; the front plate must leave to the **left** the way it arrived; Tab must reach both arrows and the rail; with `prefers-reduced-motion: reduce` nothing travels and it cross-fades in place.
5. **The footprints globe** — the canvas sphere in *Our Footprints* on `index.html`. Hand-rolled 2D orthographic projection, **no globe library and no WebGL**: land and markers pass through the same `project()`, which is the only reason the two cannot drift apart. Geometry is `assets/geo/land-110m.json` (Natural Earth, public domain) fetched at runtime, clipped ring-by-ring against the horizon. It is achromatic — land `#343437` against a `#17171b → #08080a` body, white markers dominant — and the drag gain is `1/R` radians per pointer pixel so the surface tracks the finger 1:1. Loop freezes off-screen; with reduced motion it parks at its pose and repaints only on pointer, resize or late geometry; if the fetch fails the graticule and markers still draw.
   *Verify:* on `index.html`, scroll to the plate — Sumatra through Halmahera must be recognisable with every white site dot sitting on land, not open water; drag horizontally and the surface must move exactly as far as the pointer; the palette must stay black/white/grey with no blue anywhere.
