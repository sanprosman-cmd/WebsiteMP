...
`*.min.css` / `*.min.js` are generated — never hand-edit them (`.impeccable/config.json` also excludes them from the design detector).
...
- `sitemap.xml` lists indexable pages only (`thank-you.html` is `noindex` and deliberately absent). Bump `<lastmod>` when content changes.
